...
import { setupTimeline, minimalSong } from '../helpers';

test('capture pattern editor', async ({ page }) => {
  // Navigate to the app and load test data
  ...
  if (count > 0) {
    ...
    if (box) {
      ...
      console.log(`  Space utilization: ${((box.width * box.height) / ((page.viewportSize()?.width || 1280) * (page.viewportSize()?.height || 720)) * 100).toFixed(1)}%`);
    }

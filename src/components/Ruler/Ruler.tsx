...
const Ruler = ({ viewport, onPositionClick }: RulerProps) => {
  ...
  const { bars, gridLines } = useMemo(() => {
    ...
    const gridLinesArray: Array<{ position: number; isMajor: boolean }> = [];
      ...
      if ((bar % barInterval) === 0) {
        ...
        for (let i = 1; i < 4; i++) {
          ...
          if (gridBeat >= startBeat && gridBeat <= endBeat) {
            gridLinesArray.push({ position: gridX, isMajor: false });
          }
        }
      }
    }
